...
if cmd.modelType == 'flownet' or cmd.modelType is None:
	...
	deepVO = DeepVO(cmd.imageWidth, cmd.imageHeight, cmd.seqLen, 1, activation = cmd.activation, parameterization = cmd.outputParameterization, \
		batchnorm = False, dropout = cmd.dropout, flownet_weights_path = cmd.loadModel, numLSTMCells = cmd.numLSTMCells)
...
for epoch in range(cmd.nepochs):
	...
	val_startFrames = [0]
	val_endFrames = [4500]
	kitti_train = KITTIDataset(cmd.datadir, train_seq, train_startFrames, train_endFrames,parameterization = cmd.outputParameterization, \
		width = cmd.imageWidth, height = cmd.imageHeight)
	kitti_val = KITTIDataset(cmd.datadir, val_seq, val_startFrames, val_endFrames,parameterization = cmd.outputParameterization, \
		width = cmd.imageWidth, height = cmd.imageHeight)
