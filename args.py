...
parser.add_argument('-dropout', help = 'Drop ratio of dropout at penultimate linear layer, \
	if dropout is to be used.', type = float, choices = [FloatRange(0.0, 1.0)], default = 0.1)
...
parser.add_argument('-sbatch', help = 'Replaces tqdm and print operations with file writes when \
	True. Useful for reducing I/O when not running in interactive mode (eg. on clusters)', type = bool, default = True)
